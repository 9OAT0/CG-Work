...
export default function LoginClient({
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // ใช้ replace ลด back กลับหน้า login
  ...
  return (
    <div
      ...
      style={{ backgroundImage: "url('/Rectangle 140.png')", backgroundBlendMode: "overlay" }}
    >
      {/* พื้นหลังอนุภาคแบบ client-only */}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col items-center gap-6 w-full">
          ...
          <button
            ...
            className={`w-[250px] h-[49px] rounded-[30px] mt-2 text-white font-bold text-lg transition-colors duration-300 ${
              isSubmitting ? "bg-gray-400 cursor-not-allowed" : "bg-pinkBrand hover:bg-pink-600"
            }`}
          >
