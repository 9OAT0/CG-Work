...
// ✅ Test mode toggle
const TEST_MODE = false; // true = ใช้ข้อมูล mock, false = fetch API จริง

const mockBooths: Booth[] = [
  {
    id: "1",
    booth_name: "3D Amazing Project",
    dept_type: "3D",
    description: "ตัวอย่างงาน 3D",
    pics: ["/banner.jpg"],
    booth_code: "3D001",
  ...
  {
    id: "2",
    booth_name: "Graphic Design Fun",
    dept_type: "Graphic",
    description: "งานกราฟิกสุดเจ๋ง",
    pics: ["/banner.jpg"],
    booth_code: "GRA002",
    owner_names: ["Charlie"],
    joined: true,
  ...
  useEffect(() => {
    const fetchBooths = async () => {
      ...
      try {
        if (TEST_MODE) {
          // ใช้ข้อมูล mock
          const filteredMock = mockBooths.filter(b => b.dept_type === selectedCategory);
          setBooths(filteredMock);
        } else {
      ...
      <div className="flex flex-col justify-center items-center gap-6 px-4">
        {loading ? (
          ...
          filteredBooths.map((booth) => (
            <div key={booth.id} className="w-full max-w-4xl mx-auto">
              <a href={`/booth?id=${booth.id}`}>
                <div
                  ...
                  <div className="flex-1 text-white flex flex-col justify-between">
                    ...
                    <div className="text-right">
                      {booth.owner_names && booth.owner_names.length > 0 && (
                        booth.owner_names.map((name, idx) => (
                          <p key={idx} className="text-sm mb-1">{name}</p>
                        ))
                      )}
                    </div>
                  </div>
