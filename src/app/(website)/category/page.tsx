...
function CategoryContent() {
  ...
  // Read dept from query string and map to category
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchBooths = async () => {
      setLoading(true);
      try {
        const deptType = selectedCategory;
      ...
      <div className="flex flex-col justify-center items-center gap-6 px-4">
        {loading ? (
          ...
          filteredBooths.map((booth) => (
            <a key={booth.id} href={`/booth?id=${booth.id}`} className="w-full max-w-4xl mx-auto">
              <div
                className={`w-full rounded-3xl bg-blueBrand flex flex-col lg:flex-row gap-6 p-6 relative hover:scale-105 transition-transform duration-200 ${
                  booth.joined ? "opacity-50" : ""
                }`}
              >
                {/* Image */}
                ...
                <div className="flex-1 text-white flex flex-col justify-between">
                  <div className="mb-4">
                    ...
                    <p className="text-sm text-white/80">{booth.description}</p>
                  </div>
                  <div className="text-right">
                    {booth.owner_names?.map((name, i) => (
                      <p key={`${booth.id}-owner-${i}`} className="text-sm mb-1">{name}</p>
                    ))}
                  </div>
                </div>
              </div>
